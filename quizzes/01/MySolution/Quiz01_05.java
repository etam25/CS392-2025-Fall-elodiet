...
public class Quiz01_05 {
	public static <T extends Comparable<T>> LnList<T> QuickSort(LnList<T> xs) {
		...
		result.append1(buildListFromElements(pivot, partitioned.equal));
	...
	private static <T extends Comparable<T>> PartitionResult<T> partition(LnList<T> xs, T pivot) {
		LnList<T> less = new LnList<T>();
		LnList<T> equal = new LnList<T>();
		LnList<T> greater = new LnList<T>();
	...
	private static <T> void appendToEnd(LnList<T> list, T elem) {
        LnList<T> singleElem = new LnList<T>(elem, new LnList<T>());
        if (list.nilq1()) {
	...
	private static <T> LnList<T> buildListFromElements(T first, LnList<T> rest) {
        return new LnList<T>(first, rest);
    }
	...
	public static void main(String[] args) {
        ...
        LnList<Integer> random = new LnList<Integer>(3, 
                                 ...
                                 new LnList<Integer>(4, 
                                 new LnList<Integer>(1, 
                                 new LnList<Integer>(5, 
                                 new LnList<Integer>(9, 
                                 new LnList<Integer>(2, new LnList<Integer>())))))));
