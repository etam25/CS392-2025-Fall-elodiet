public class Quiz02_02 {
    ...
    void sort1000WithNoRecursion(T[] A) {
        ...
        // iteratively merge subarrays of increasing sizes: 1, 2, 4, 8, 16, ...
        int n = A.length;
        if (n <= 1) return;

        T[] temp = (T[]) new Comparable[n];

        // start w/ subarrays of size 1, double each iteration
        // build a "tree" of merges from bottom to top
        for (int size = 1; left < n - size; left += 1 * size) {
