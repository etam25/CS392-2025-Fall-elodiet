...
public class FnListSUtil {
    ...
    public static<T> FnList<T> sing(T x0) {
        return cons(x0, nil());
    }
    ...
    public static FnList<Object> int0$make(int n0) {
    ...
    public static FnList<Integer> int1$make(int n0) {
    ...
    public static FnList<Integer> rand$int$make(int n0) {
    ...
    public static<T> LnStrm<T> strmize(FnList<T> xs) {
        return new LnStrm<T> (
            () -> {
                if (xs.nilq()) {
                    return new LnStcn<T>();
    ...
    public static<T extends Comparable<T>> int x2forcmp(FnList<T> xs, FnList<T> ys) {
        return xs.U0.z2forcmp(xs, ys, (x0, y0) -> x0.compareTo(y0));
    }

    public static<T> FnList<T> fwork$make(Consumer<Consumer<? super T>> fwork) {
